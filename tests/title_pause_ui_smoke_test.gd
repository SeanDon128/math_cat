...
func _test_title_hub() -> void:
    ...
    title._focus_detail(4)
    var cat_color_button: Button = title.detail_buttons[4] as Button
...
func _test_pause_actions() -> void:
    ...
    assert(game._pause_options_buttons.size() == 7, "Pause Options must expose six settings and a Back action.")
