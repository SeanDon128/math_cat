class_name GradeTenGenerator
extends RefCounted

const QUESTION_BANK_PATH := "res://data/questions/grade_10.json"
...
func _refill_question_deck() -> void:
    ...
    assert(file != null, "Unable to open Grade 10 question bank.")
    var parsed = JSON.parse_string(file.get_as_text())
    assert(parsed is Array, "Grade 10 question bank must be a JSON array.")
    for record in parsed:
        assert(record is Dictionary, "Each Grade 10 question record must be an object.")
        _unused_questions.append(record)
    assert(_unused_questions.size() == 570, "Grade 10 question bank must contain 570 questions.")
