class_name GradeTwelveGenerator
extends RefCounted

const QUESTION_BANK_PATH := "res://data/questions/grade_12.json"
...
func _refill_question_deck() -> void:
    ...
    assert(file != null, "Unable to open Grade 12 question bank.")
    var parsed = JSON.parse_string(file.get_as_text())
    assert(parsed is Array, "Grade 12 question bank must be a JSON array.")
    for record in parsed:
        assert(record is Dictionary, "Each Grade 12 question record must be an object.")
        _unused_questions.append(record)
    assert(_unused_questions.size() == 585, "Grade 12 question bank must contain 585 questions.")
