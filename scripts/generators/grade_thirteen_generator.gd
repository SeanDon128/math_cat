class_name GradeThirteenGenerator
extends RefCounted

const QUESTION_BANK_PATH := "res://data/questions/grade_13.json"
...
func _refill_question_deck() -> void:
    ...
    assert(file != null, "Unable to open Grade 13 question bank.")
    var parsed = JSON.parse_string(file.get_as_text())
    assert(parsed is Array, "Grade 13 question bank must be a JSON array.")
    for record in parsed:
        assert(record is Dictionary, "Each Grade 13 question record must be an object.")
        _unused_questions.append(record)
    assert(_unused_questions.size() == 520, "Grade 13 question bank must contain 520 questions.")
