...
static func save_profile(draft: Dictionary) -> String:
    ...
    ProfileManager.apply_display_settings()
...
static func validate_profile(draft: Dictionary) -> String:
    ...
    if draft.settings.controller_input_style not in ["direct", "navigation"] or not ProfileManager.CAT_COLORS.has(draft.settings.cat_color) or not draft.settings.animated_background is bool or not draft.settings.fullscreen is bool:
        return "Invalid controller style, cat color, background, or fullscreen setting."
